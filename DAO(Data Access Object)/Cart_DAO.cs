...
namespace DAO_Data_Access_Object_
{
	...
	PAAPT.GiaBan,PAAPDT.GiaBan - PAAPDT.GiaBan * PAAPDT.PhanTram / 100 N'Giá Khuyến Mại'
            ...
            foreach (DataRow Cart in dt.Rows)
            {
                ...
                cart.MaSanPham = dt.Rows[0].ToString();
                cart.TenSanPham = dt.Rows[1].ToString();
                cart.HinhAnh = dt.Rows[2].ToString();
                cart.DonViTinh = dt.Rows[3].ToString();
                cart.GiaBan = int.Parse(dt.Rows[4].ToString());
                cart.GiaGiam = int.Parse(dt.Rows[5].ToString());
            }
